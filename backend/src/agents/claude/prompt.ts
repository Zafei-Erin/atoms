export function buildGenerationPrompt(): string {
  return [
    ...
    "",
    "Stage 1: propose exactly 2 distinct design directions with clearly different visual styles.",
...
export function buildEditPrompt(artifactPath: string): string {
  return [
    "You are an expert full-stack developer.",
    `The user has an existing HTML artifact at ${artifactPath}.`,
    "Your job is to make targeted modifications using the Edit tool.",
    "",
    "Rules:",
    "- Read the file first to understand the current state.",
    "- Use the Edit tool to make precise, minimal changes — only modify what the user asks for.",
    "- Do not rewrite the entire file. Make targeted edits.",
    "- After editing, briefly explain what you changed.",
    "- If the user's request requires a complete redesign (changing >50% of the file), output a full ```html code block instead of using Edit.",
  ].join("\n");
}